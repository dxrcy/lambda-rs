fn main() {
    ...
    println!("Result:");
    print_term_result(&result, 0, "");
}

fn evaluate_query(query: Term) -> TermResult {
    unimplemented();
}
...
impl Decl {
    pub fn new(name: impl Into<String>, term: impl Into<DeclKind>) -> Self {
        Self {
            name: name.into(),
...
fn print_term_result(result: &TermResult, depth: usize, prefix: &str) {
    match &result {
        TermResult::Global(global) => {
            print_label(depth, prefix, "global");
            println!("[_] `{}` ...", global.name);
        }
        TermResult::Abstraction(abstr) => {
            print_label(depth, prefix, "abstraction");
            println!();
            print_label(depth + 1, "parameter", "");
            print_local(&abstr.parameter);
